...
이 구조적 이해상충을 풀려고 등장한 게 **MMP(Mobile Measurement Partner)**다. 앞선 [유저 제스처와 딥링크 글](/ad-tech/deep-link/2026/06/20/user-gesture-deep-link/)에서 "딥링크 처리는 끝단 MMP의 몫"이라고만 짚고 넘어갔는데, 이번엔 그 MMP가 정확히 무엇을 하는 물건인지 정리한다. AppsFlyer·Adjust·Branch·Singular·Airbridge 같은 이름들이 모두 이 범주다.

---

## MMP는 중립 심판이다

MMP를 한 줄로 정의하면 **광고 클릭·노출을 앱 설치·인앱 이벤트에 매칭하는 중립적 제3자 측정 플랫폼**이다. [AppsFlyer의 정의](https://www.appsflyer.com/glossary/mmp/)를 빌리면 "캠페인 성과를 균일하게 평가하기 위해 앱 데이터를 어트리뷰션·수집·정리하는 플랫폼"이고, 핵심은 **중립성**이다 — 경쟁하는 광고 네트워크들 사이에서 "어느 매체에 크레딧을 줄지 판정하는 공정한 심판" 역할.
...
어트리뷰션의 본질은 의외로 단순하다. **engagement(클릭/노출)를 install과 그 이후 인앱 이벤트(가입·구매 등)에 연결**하는 매칭이다. "이 설치는 어제 그 클릭에서 왔다"를 알아내는 것. 어떻게 두 사건을 같은 사람의 것이라고 묶느냐 — 여기서 방식이 셋으로 갈린다.

### 1. Deterministic — ID/referrer로 1:1 매칭

가장 정확한 방식. 광고 식별자(안드로이드 GAID, iOS IDFA)나 click ID를 클릭과 설치 양쪽에서 확인해 **결정적으로** 묶는다.

안드로이드에는 특히 강력한 수단이 있다 — **Google Play Install Referrer**. 동작은 이렇다.
...
결정적 ID가 없을 때만 쓰는 **폴백**이다. 클릭 시점에 device "snapshot"을 찍고, 설치 후의 snapshot과 **확률적으로 대조**한다. [Branch의 표현](https://www.branch.io/resources/blog/deferred-deep-linking-with-device-snapshotting/)으로는 snapshot은 "타임스탬프, IP 주소, OS, OS 버전, 기기 모델, 화면 크기 등"의 특성 집합이다. 둘이 충분히 일치하면 같은 사람으로 본다.
...
[Airbridge의 정리](https://www.airbridge.io/en/blog/deferred-deeplink-implementation-guide-for-ios-android)대로 이건 **2단계 프로세스**다.
...
중요한 디테일 하나 — **매칭 시간 창은 provider별로 다르고 설정 가능하다.** [Branch는 기본 2시간](https://www.branch.io/resources/blog/deferred-deep-linking-with-device-snapshotting/)(클릭 후 앱 오픈의 99% 이상이 60분 내에 일어나기 때문), Airbridge는 약 1시간, AppsFlyer의 lookback은 15분. 이 창을 넘겨서 설치하면 저장된 목적지는 버려지고 기본/홈 화면으로 떨어진다. "약 1시간"을 보편 상수처럼 외우면 안 되는 이유다.

---

## 프라이버시 변화가 흔든 것

위 매칭 방식들은 지난 몇 년간 프라이버시 정책에 직격탄을 맞았다. MMP를 이해하려면 이 흐름을 반드시 알아야 한다.

- **Apple ATT (App Tracking Transparency, 2021)** — IDFA를 opt-in으로 바꿨다. 유저가 허락해야 광고 식별자를 쓸 수 있으니, **deterministic ID 매칭이 대폭 약화**됐다.
- **iCloud Private Relay (iOS 15+)** — Safari·인앱 Safari에서 IP를 가린다. fingerprint의 핵심 신호인 IP가 사라지니 **probabilistic 매칭도 약화**된다. (iOS 17은 fingerprinting 자체를 더 제약했다.)
...
- **MMP는 광고 네트워크들의 자기 보고를 못 믿어서 세운 중립 심판**이다. 클릭·노출을 설치·이벤트에 매칭(어트리뷰션)하고, 그 과정에서 딥링크를 끝단에서 처리한다.
...
"어떤 광고가 이 설치를 만들었나"라는 단순한 질문 하나가, 식별자·딥링크·프라이버시 정책이 얽힌 이 모든 메커니즘을 끌고 들어온다. MMP는 그 질문에 답하려고 존재하는 레이어다 — 그리고 그 답의 정확도는, 우리가 프라이버시를 더 지킬수록 흐려진다.
