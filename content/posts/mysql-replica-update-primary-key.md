...
새벽 1시 42분, 알림 발송 SaaS 회사 **오비트포스트(OrbitPost)**의 당직 엔지니어에게 경보가 왔다.
...
오비트포스트는 이 글을 위해 만든 가상의 회사다. 테이블, 시각, 식별자와 관측 수치도 실제 조사 경험을 바탕으로 전부 바꿔 재구성했다. 다만 MySQL의 동작과 진단 방법은 MySQL 8.4 기준이다.

## 사고 전의 구조

오비트포스트는 고객사를 대신해 이메일과 푸시 알림을 보낸다. 발송 요청은 `event_delivery_history`에 쌓이고, 전송 결과가 돌아오면 `pending` 상태를 `completed` 또는 `failed`로 바꾼다.

테이블은 월간 파티션으로 관리했고 전체 row 수는 약 1억 4천만 건이었다.

```sql
CREATE TABLE event_delivery_history (
...
```sql
UPDATE event_delivery_history
SET status = 'completed', updated_at = NOW()
WHERE recipient_id = 42001
  AND status = 'pending'
  AND created_at >= '2026-07-20 00:00:00';
```

source에서 이 쿼리는 빨랐다. `idx_recipient_created`로 대상 범위를 좁혀 몇 건만 수정했다. 그래서 처음에는 이 UPDATE가 Replica 장애의 원인일 것이라고 생각하지 못했다.
...
오비트포스트의 Replica는 IO thread가 정상적으로 binlog를 받고 있었다. 반면 SQL applier 쪽에는 lock wait timeout이 남아 있었다.
...
문제 트랜잭션은 수십 MB짜리 배치가 아니었다. `event_delivery_history`에서 발송 이력 6건의 상태만 바꾸는 작은 `Update_rows` 이벤트였다.

```text
### UPDATE `app`.`event_delivery_history`
### WHERE
###   @1=17
###   @2=42001
###   @3='evt-001'
...
오비트포스트는 `binlog_format=ROW`를 사용했다. 이 방식에서 Replica는 source의 UPDATE 문이나 실행 계획을 전달받지 않는다.
...
source가 `idx_recipient_created`로 빠르게 UPDATE했다는 사실은 Replica의 row 탐색 경로를 보장하지 않았다. Replica는 자기 테이블의 인덱스와 row image만 보고 변경 대상을 다시 찾아야 했다.
...
```sql
SHOW CREATE TABLE app.event_delivery_history\G
```

PK도 없고 모든 컬럼이 `NOT NULL`인 UNIQUE 키도 없었다. 일반 인덱스 중 왼쪽에 있는 `idx_created_at`이 Replica row search에 선택될 수 있는 구조였다.
...
몇 개의 로그 테이블이 더 나왔지만 모두 같은 위험도를 가진 것은 아니었다. 한 테이블은 INSERT만 발생했고, `event_delivery_history`에는 지속적인 UPDATE가 들어왔다.
...
오비트포스트에서는 worker들이 외부 세션 하나를 기다리는 것이 아니라 서로 다른 applier 트랜잭션을 재시도하는 모습이었다. 이제 실제 InnoDB lock을 확인할 차례였다.
...
WHERE object_schema = 'app'
  AND object_name = 'event_delivery_history'
GROUP BY 1, 2, 3, 4, 5
ORDER BY lock_count DESC;
```

락은 `idx_created_at`과 `GEN_CLUST_INDEX`에 집중되어 있었다.

```text
index_name       lock_type  lock_mode      lock_count
idx_created_at   RECORD     X,REC_NOT_GAP         906
...
대기 관계의 양쪽이 모두 applier worker였고, 같은 `idx_created_at` 구간에서 대기 체인이 만들어졌다.
...
RBR applier가 UPDATE 대상 row를 point lookup하지 못함
  ↓
idx_created_at을 순회하며 before image와 대조
  ↓
...
worker끼리 충돌하는 것이 직접 확인된 뒤 오비트포스트는 `replica_parallel_workers`를 일시적으로 1로 낮췄다. 병렬 처리량을 포기하는 대신 worker 간 대기 체인을 없애는 선택이었다.
...
오비트포스트는 먼저 자연키를 검토했다.

```sql
ALTER TABLE event_delivery_history
ADD CONSTRAINT uk_event_delivery_external
UNIQUE (external_event_id, created_at);
```

파티션 테이블의 모든 UNIQUE 키는 파티션 표현식에 사용되는 컬럼을 포함해야 하므로 `created_at`도 넣었다. 하지만 과거 데이터에 중복 `external_event_id`가 있었고, 재시도 요청을 중복 이력으로 보관하는 업무 규칙도 있었다. 자연키를 UNIQUE로 만들면 기존 DDL이 실패하거나 정상 INSERT가 duplicate key error로 바뀐다.
...
`created_at`은 파티션 키 제약 때문에 PK에 포함했다. 짧은 surrogate key를 택한 이유도 있다. InnoDB의 secondary index에는 clustered primary key 값이 함께 저장된다. 긴 자연키를 PK로 쓰면 기존 secondary index 전체가 커질 수 있다.

수억 row 테이블에 PK를 추가하는 DDL은 그 자체로 위험하다. 테이블 재구성 시간, 추가 디스크 공간, metadata lock과 쓰기 영향을 확인해야 한다. 오비트포스트는 운영 테이블에 즉시 ALTER하지 않고, PK가 있는 새 테이블을 만든 뒤 쓰기와 읽기를 순차적으로 전환했다.

PK 적용 후에는 같은 `Update_rows`가 point lookup으로 처리됐고, 비정상적인 lock 증폭과 transaction retry 증가가 멈췄다. 그 뒤 병렬 worker 수를 원래 값으로 돌렸다.

## 다음 장애에서 바로 실행할 순서

이 사건 이후 오비트포스트는 Replica lag 대응 순서를 다음처럼 정리했다.
...
에러 로그만으로는 과거의 lock timeout을 알 수 있지만, 당시 누가 어떤 인덱스에서 누구를 막았는지까지 직접 증명하기 어렵다. 오비트포스트는 lag 경보가 발생하면 다음 결과를 자동으로 저장하도록 했다.
...
오비트포스트의 장애에서 가장 헷갈렸던 부분은 source의 UPDATE가 빠르고 Replica의 CPU와 IOPS도 낮았다는 점이었다. 하지만 row-based replication에서 source의 실행 계획은 Replica의 실행 경로가 아니다. Replica는 row event를 적용하기 위해 변경 대상을 자기 테이블에서 다시 찾아야 한다.
