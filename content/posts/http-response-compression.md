...
HTTP 압축은 서버가 응답 본문을 압축해서 전송하고, 클라이언트가 이를 풀어서 사용하는 구조다. 헷갈리는 지점은 압축 코드가 앱 서버에 없을 때다. 실제로는 앱, 프록시, CDN 중 어느 레이어든 `Content-Encoding`을 붙일 수 있다.
...
`q=1.0`은 선호도 상한이다. 서버는 이걸 참고해서 알고리즘을 고를 수 있다. 실제로는 대부분의 서버가 `q` 값을 무시하고 자기가 지원하는 방식 중 압축률이 높은 쪽을 선택한다.
...
서버가 압축을 지원하지 않거나, 응답이 작아서 압축 이득이 없다고 판단하면 `Content-Encoding` 없이 원본을 보낸다. 클라이언트는 헤더 유무로 판단하므로 어느 쪽이든 같은 방식으로 처리한다.
...
Go 표준 라이브러리 `transport.go`의 주석은 이렇게 적고 있다:
...
제약이 하나 있다: **브라우저는 HTTPS 연결에서만 `br`을 요청한다.** 이유는 두 가지다. HTTP 평문 연결에서는 중간 프록시가 `br`을 인식하지 못해 응답이 깨질 수 있고, 브라우저 벤더들이 새 전송 기능을 HTTPS에서만 활성화하는 정책("Encrypt the Web")을 밀고 있기 때문이다. curl 같은 비브라우저 클라이언트는 HTTP에서도 `br`을 사용할 수 있다.
...
- **범용 API 서버**: gzip이 무난하다. 모든 클라이언트가 지원하고 설정이 간단하다.
...
Go의 HTTP Transport는 무조건 gzip을 요청하지 않는다. **4가지 조건을 모두 만족할 때만** 자동으로 `Accept-Encoding: gzip`을 붙인다.
...
이 설계는 사용자가 명시적으로 헤더를 설정했다면 "직접 다루겠다"는 의도로 간주한다. 자동 디코딩이 사용자의 설정을 덮지 않는다.
...
`gzip_vary on`이 추가하는 `Vary: Accept-Encoding` 헤더는 캐시 정합성을 좌우한다. CDN이나 프록시가 같은 URL에 대해 압축된 버전과 비압축 버전을 구분해서 캐싱할 수 있게 해준다. 이 헤더가 없으면 gzip을 지원하지 않는 클라이언트에게 압축된 응답이 전달되는 캐시 오염이 발생할 수 있다.

이미 압축된 파일(JPEG, PNG, ZIP 등)이나 너무 작은 응답은 압축해도 효과가 없거나 오히려 커질 수 있다. `gzip_min_length`와 `gzip_types`로 대상을 적절히 제한해야 한다.

### 앱 서버가 아닌 앞단에서 처리하라

서비스에서는 보통 앱 서버가 아닌 **리버스 프록시나 CDN**에서 처리한다. 앱 서버는 비즈니스 로직에 집중하고, 압축 같은 전송 최적화는 앞단에 맡긴다.
