...
같은 숫자들을 더했는데 순서를 바꾸면 결과가 달라질 수 있다. 부동소수점이 실수를 정확히 들고 있는 게 아니라, 제한된 비트 안에 근사해서 담기 때문이다. 특히 큰 수와 작은 수를 섞어 더할 때 작은 쪽의 정보가 잘려 나간다.
...
**손실이 줄어드는 순서: 비슷한 크기끼리 먼저 더하기 (pairwise summation)**
...
덧셈처럼 극적이지는 않지만, 곱셈에서도 순서가 영향을 준다. 볼 지점은 **중간 결과의 오버플로우/언더플로우 방지**다.
...
주의: `new BigDecimal(0.1)`처럼 `double`에서 변환하면 이미 오염된 값이 들어간다. **문자열로 초기화**해야 한다.
...
double의 가수부는 52비트다. 연산 순서를 바꿀 때의 목표는 이 52비트 안에 의미 있는 하위 비트를 최대한 남기는 것이다. 금융처럼 십진 정확성이 요구되는 계산에서는 애초에 float 대신 정수나 decimal 타입을 써야 한다.
