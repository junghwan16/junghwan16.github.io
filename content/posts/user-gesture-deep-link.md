...
위 메커니즘을 이해하면, 업계에서 통용되는 몇 가지 원칙이 왜 그렇게 굳었는지가 보인다. 한 줄로 요약하면 **"진짜 탭의 권한이 살아 있는 동안, 가능한 한 적은 홉으로 앱 오픈에 도달하라"**다.

- **홉이 적을수록 좋다.** 리다이렉트가 한 단 줄면 시간 창 안에 들어올 확률과 클릭 컨텍스트가 보존될 확률이 동시에 올라간다. 그래서 트래킹 링크를 설계할 때 불필요한 중간 단계를 줄이는 게 기본으로 권장된다.
...
> 이 원칙들의 인과 사슬 자체("302 체인 → 만료/소진/컨텍스트 손실 → 깨짐")는 명세와 벤더 공개 문서에서 연역한 합성적 결론이다. 단일 1차 출처가 이 문구 그대로 검증하지는 않으니, 신뢰도는 "원리는 high, 구체 처방은 medium" 정도로 잡는 게 정직하다.
