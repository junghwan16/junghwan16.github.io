---
title: "OpenRTB 트래킹 URL은 누가 호출할까"
url: "/ad-tech/openrtb/2026/02/24/openrtb-tracking/"
...
OpenRTB 트래킹을 볼 때 제일 헷갈리는 질문은 하나다. 이 URL을 누가 호출하는가? `nurl`, `burl`, VAST Impression, 픽셀 URL은 비슷하게 생겼지만 호출 주체와 시점이 다르다. 이걸 섞어 이해하면 과금 누락, 중복 집계, 트래킹 실패가 생긴다.

먼저 서버가 호출하는 URL과 클라이언트가 호출하는 URL을 나누고, 그 다음 Banner, Video, Native별로 트래킹이 어디서 발생하는지 본다.
...
주의할 점은 `nurl`이 광고 노출이 아니라는 것이다. 유저가 실제로 광고를 봤는지는 이 시점에 알 수 없다. `nurl`만으로 과금하면 안 된다.
...
nurl/burl/lurl의 세부 사항(BidResponse 필드 정의, 매크로 목록 등)은 BidResponse 글에서 다룬다. 여기서는 호출 주체와 시점에 집중한다.
...
Banner는 노출 트래킹은 쉽지만 클릭 트래킹이 까다롭다. DSP가 만든 HTML 안의 클릭 URL을 SSP가 어디까지 믿고 고칠 수 있느냐의 문제다.
...
Banner만 어려운 이유는 다른 미디어 타입과 비교하면 보인다:
...
| 장점 | 단점 |
|------|------|
| DSP 협조 불필요 | HTML 파싱/수정 필요 (구현 부담) |
| 정확한 집계 | 리다이렉트 체인 길어짐 (레이턴시) |
| | 임의 HTML/JS에서 href 파싱 실패 가능 |
...
Native 광고의 adm은 구조화된 JSON이다. 데이터(assets)와 트래킹(imptrackers, clicktrackers)이 분리되어 있어 트래커를 넣기 수월하다.
...
| 미디어 타입 | 노출 (Impression) | 클릭 (Click) |
|-----------|------------------|-------------|
| **Native** | 매우 쉬움 — `imptrackers[]`에 URL 추가 | 매우 쉬움 — `clicktrackers[]`에 URL 추가 |
| **Video** | 쉬움 — `<Impression>` 태그 추가 | 쉬움 — `<ClickTracking>` 태그 추가 |
| **Banner** | 쉬움 — 1x1 `<img>` 태그 추가 | **어려움** — `<a href>` 하드코딩, Click Macro 등 필요 |
...
Click Macro를 먼저 본다. DSP가 `${CLICK_URL}` 매크로를 지원하면 SSP가 서버 사이드에서 치환한다. 미지원 DSP에는 Redirect Wrapping을 적용한다. 네 가지 방식의 비교는 위의 Banner 클릭 트래킹 섹션에 있다.
...
- [OpenRTB 2.6 Specification (IAB Tech Lab)](https://github.com/nicholasgasior/openrtb/blob/master/OpenRTB-API-Specification-Version-2-6.pdf) — nurl, burl, lurl, adm, 매크로 정의
- [IAB Tech Lab OpenRTB](https://iabtechlab.com/standards/openrtb/) — OpenRTB 표준 페이지
- [IAB VAST 4.2 Specification](https://iabtechlab.com/standards/vast/) — Impression, TrackingEvents, VideoClicks, Wrapper 구조
- [OpenRTB Dynamic Native Ads API 1.2](https://iabtechlab.com/standards/openrtb-native/) — imptrackers, clicktrackers, eventtrackers 정의
- [IAB MRAID 3.0](https://iabtechlab.com/standards/mraid/) — mraid.open() 동작
- [IAB SIMID 1.1](https://iabtechlab.com/standards/simid/) — VPAID 대체 규격
