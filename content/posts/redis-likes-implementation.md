...
뉴스 댓글에 좋아요 기능을 붙인다고 해보자. 평소에는 `like_count = like_count + 1`이면 될 것처럼 보인다. 문제는 인기 댓글 하나에 요청이 몰리는 순간이다. 같은 row를 계속 업데이트하면 DB는 카운터가 아니라 락 대기열처럼 보이기 시작한다.
...
인기 댓글에 초당 수천 개의 좋아요 요청이 몰리면 이런 모양이 된다.
...
| **하이브리드** | 성능 + 안정성 | 운영 부담, 불일치 가능 | **대부분의 상용 서비스** |

트래픽이 작은 서비스는 RDB 단독으로도 버틴다. 좋아요가 특정 객체에 몰리는 서비스라면 Redis로 쓰기 경합을 빼고, RDB에는 배치로 반영하는 구조가 운영 부담이 적다.
...
DAU처럼 정확한 숫자보다 추세를 보는 경우 HyperLogLog가 메모리를 훨씬 적게 쓴다:
