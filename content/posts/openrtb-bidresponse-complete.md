...
url: "/ad-tech/openrtb/2026/02/24/openrtb-bidresponse-complete/"
...
| `w` | int | 권장 | 광고 너비 (픽셀) |
| `h` | int | 권장 | 광고 높이 (픽셀) |
| `exp` | int | 선택 | 입찰 만료 시간 (초). 스펙에서 기본값을 정의하지 않으며, 일반적으로 3600초 전후 값이 사용됨 |
| `ext` | object | 선택 | 확장 필드 |
...
`${AUCTION_PRICE}`는 평문 전송 시 중간자 공격에 노출될 수 있다. 실제 구현에서는 암호화 후 Base64URL 인코딩된 형태로 전송하는 것이 일반적이나, 암호화 방식은 Exchange마다 다르다 (예: Google은 자체 커스텀 방식 사용).
...
- [OpenRTB 2.6 Specification (IAB Tech Lab GitHub)](https://github.com/InteractiveAdvertisingBureau/openrtb2.x)
- [IAB Tech Lab - OpenRTB](https://iabtechlab.com/standards/openrtb/)
- [IAB VAST 4.2](https://iabtechlab.com/standards/vast/)
- [OpenRTB Native Ads 1.2](https://iabtechlab.com/standards/openrtb-native/)
