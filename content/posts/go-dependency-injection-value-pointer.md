---
title: "Go 의존성 주입에서 값과 포인터를 어떻게 넘길까"
...
단순 데이터면 value가 자연스럽다.

공유 리소스나 내부 상태가 있으면 pointer가 자연스럽다.
...
Go에서 의존성 주입은 프레임워크보다 타입 선택이 중요하다.

인터페이스는 소비하는 쪽에서 작게 정의하고 value로 받는다. 실제 구현체는 보통 pointer로 만들고, 단순 데이터는 value로 넘긴다. 이 기준만 있어도 handler, service, repository 사이의 경계가 꽤 깔끔해진다.
