---
title: "Dynamo 장바구니가 사과와 오렌지를 둘 다 살리는 법"
url: "/backend/infrastructure/2026/02/16/vector-clock-conflict-resolution/"
date: 2026-02-16 18:00:00 +0900
categories: [backend, infrastructure]
---

장바구니에 사과를 넣는 요청과 오렌지를 넣는 요청이 거의 동시에 들어왔다고 해보자. 네트워크가 갈라져 있어서 서로 다른 노드가 각각 요청을 받았다면 어느 쪽을 남겨야 할까?
...
Dynamo는 이 상황에서 한쪽을 조용히 버리지 않는다. 벡터 클럭으로 충돌을 감지하고, 애플리케이션이 두 버전을 병합하게 한다.
...
- 이게 **앞선 업데이트의 후속**인지 (한 쪽이 다른 쪽을 포함)
...
V1의 모든 카운터가 V2 이하다. V1은 과거 버전이므로 제거된다.
...
장바구니라면 합집합(union)이 비즈니스 규칙에 맞다.
...
하나만 고를 수 없다. 논문에서도 "remove된 아이템이 다시 나타날 수 있다"고 언급한다.
...
**R + W > N**이면 읽기 노드 집합과 쓰기 노드 집합이 겹친다.
