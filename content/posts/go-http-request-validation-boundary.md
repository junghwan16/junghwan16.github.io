...
핸들러에서 전부 해야 할까? 서비스 레이어로 넘겨야 할까? `go-playground/validator` 같은 라이브러리를 바로 붙여야 할까? 처음에는 단순한 질문처럼 보이지만, 이 경계를 잘못 잡으면 HTTP 코드와 비즈니스 코드가 쉽게 섞인다.
...
하지만 시간이 지나면 HTTP의 사정이 애플리케이션 내부 타입으로 새기 쉽다. JSON field 이름, optional field 정책, 외부 API 호환성 같은 것들이 usecase 타입에 묻어난다.
...
초기 HTTP 서버 구조를 잡는 단계라면 얕은 검증이면 충분하다.
...
DTO를 command로 바꿀 때 slice를 다루다가 이런 코드를 쓰기 쉽다.
...
이 정도 경계만 지켜도 HTTP 서버 구조가 훨씬 덜 엉킨다. 중요한 건 validation을 얼마나 많이 하느냐가 아니라, 어떤 책임을 어느 레이어에 둘지 일관되게 유지하는 것이다.
