...
Istio에서 Gateway와 VirtualService는 역할이 분리되어 있다.

- **Gateway**: 외부 트래픽을 수신하는 **리스너**를 정의한다 (포트, 프로토콜, TLS, 호스트명).
- **VirtualService**: 수신된 트래픽을 **어디로, 어떻게 라우팅**할지 규칙을 정의한다.
...
- [Istio Traffic Management 개념](https://istio.io/latest/docs/concepts/traffic-management/) — VirtualService, Gateway, DestinationRule 관계
