...
Go의 에러 처리는 쉬워 보인다. `if err != nil { return err }`. 하지만 이게 전부라고 생각하면 운영 중에 원인을 찾기 어려운 에러를 만나게 된다.
...
대부분의 Go 코드가 둘 중 하나를 놓친다. 둘을 구분하면 어떤 에러를 감싸고, 어떤 에러를 분기용으로 노출할지 정하기 쉬워진다.
...
각 레이어는 에러를 그냥 위로 넘기지 말고, 자신이 알고 있는 정보를 덧붙인다.
...
에러를 어떤 형태로 만들지는 호출자가 이 에러로 무엇을 하려는지에 달려 있다.
...
분기만 필요하다면 이 정도면 된다. 에러 타입을 따로 만들 필요가 없다.
...
타입 에러로 시작하면 호출자와 정의 사이에 결합도가 생긴다. 구조가 바뀌면 양쪽을 같이 수정해야 한다. 분기만 필요하다면 sentinel이 맞다.
...
sentinel과 타입 에러를 구분했다면 Is/As는 자동으로 정해진다.
...
내부 에러를 HTTP 응답으로 어떻게 변환하느냐에 따라 API 계약이 달라진다. 잘못하면 두 가지 문제가 생긴다.
...
**원칙**: 에러는 **한 번만** 로그한다. 바깥 경계(handler)에서 로그하고, 안쪽 레이어는 래핑만 한다.
...
에러 처리는 보일러플레이트가 아니다. 어떻게 처리하느냐가 시스템의 디버거 친화성, API 계약, 레이어 간 결합도를 결정한다.

- `%w`로 래핑하면 디버거는 새벽 3시에도 문제를 찾을 수 있다.
- sentinel/타입 에러를 목적에 맞게 쓰면 호출자 분기가 드러난다.
