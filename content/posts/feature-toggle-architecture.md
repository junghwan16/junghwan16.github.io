...
```
PM: "비율 25%에서 50%로 올려주세요"
         │
         ▼
개발자: 코드 수정 → PR 생성 → 코드 리뷰 → 머지 → 배포
         │
         ▼
    (수 시간 ~ 수 일 소요)
```
...
```
┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐
│Service A │  │Service B │  │Service C │  │ ... ×100 │
└────┬─────┘  └────┬─────┘  └────┬─────┘  └────┬─────┘
     │             │             │              │
     └──────────┬──┴─────────┬──┘──────────────┘
                │            │
...
│  │                    SDK (In-Process)                │       │
│  │                                                   │       │
│  │  GetVariable("myFeature", facets)                 │       │
│  │       │                                           │       │
│  │       ▼                                           │       │
│  │  ┌─────────────────┐   ┌──────────────────────┐  │       │
│  │  │  In-Memory Model │   │  Background Poller   │──┼──→ S3 │
│  │  │  (sub-μs 조회)   │←──│  (주기적 설정 fetch) │  │       │
│  │  └─────────────────┘   └──────────────────────┘  │       │
│  │                                                   │       │
│  │  Track(event, facets)                             │       │
│  │       │                                           │       │
│  │       ▼                                           │       │
│  │  ┌──────────────────────────────────┐             │       │
│  │  │  Event Buffer (Binary Batch)     │─────────────┼──→ Data Pipeline
│  │  └──────────────────────────────────┘             │       │
│  └───────────────────────────────────────────────────┘       │
└──────────────────────────────────────────────────────────────┘
```
...
Background:  [S3 fetch]────────[S3 fetch]────────[S3 fetch]───
                ↓                   ↓                   ↓
In-Memory:   [config v1]────────[config v2]────────[config v3]
                ↑       ↑       ↑   ↑       ↑
App calls:   GetVar  GetVar  GetVar GetVar GetVar
             (~0.1μs) (~0.1μs)  ... 전부 메모리 읽기
```
...
│   2 → "myFeature"                                    │
│                                                     │
│ Events (variable-size integer encoding):             │
│   [0:123, 1:10, 2:...]                              │
│   [0:123, 1:10, 2:...]                              │
│   [0:456, 1:10, 2:...]                              │
│                                                     │
│ → 문자열은 한 번만, 이후 정수 ID로 참조                │
└─────────────────────────────────────────────────────┘
```
