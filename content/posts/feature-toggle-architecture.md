...
팀이 10개이고 동시에 돌리는 실험이 50개라면, 실험 비율 하나 바꾸는 데 코드 수정 → PR → 리뷰 → 배포 사이클을 매번 돌릴 수 없다. Grab은 이 문제를 feature toggle SDK로 풀었다.
...
팀이 커지면서 "endless meetings"과 수 주간의 반복이 발생했다. 실험 50개를 동시에 운영하면서 각각의 비율을 코드로 관리한다고 생각해보면, 왜 이 방식이 오래 버티기 어려운지 보인다.
...
Grab은 이 문제를 SDK 기반 아키텍처로 해결했다. 설계가 보는 지점은 하나다. **`GetVariable()` 호출 시 네트워크 I/O가 전혀 발생하지 않는다.**
...
**JSON 전송** — 매 이벤트마다 키 이름과 문자열이 반복된다:
...
Grab의 사례에서 뽑을 수 있는 설계 기준들이다.

### 1. 핫패스에서 네트워크를 제거하라

`GetVariable()`이 요청 처리 경로(hot path)에 있다면, 네트워크 호출은 바로 레이턴시가 된다. 평가는 프로세스 안의 메모리에서 끝나야 한다.
