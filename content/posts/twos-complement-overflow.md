---
title: "-INT_MIN은 왜 다시 INT_MIN이 될까"
...
32비트 signed int에서 최솟값을 부정하면 다시 자기 자신이 된다. 이상해 보이지만 2의 보수에서는 자연스러운 결과다. 음수 쪽 값이 양수 쪽보다 하나 더 많기 때문이다.
...
`abs()` 함수도 내부적으로 부정 연산을 수행하므로 동일한 문제가 발생한다. 외부 입력에 대해 `abs()`를 호출할 때는 먼저 `INT_MIN` 여부를 확인해야 한다.
