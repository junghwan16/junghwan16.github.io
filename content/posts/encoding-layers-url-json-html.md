...
> Go 서버가 내려준 URL에서 query parameter가 통째로 사라지는 버그를 만났다. 원인을 추적하다 보니, 웹 개발에서 자주 헷갈리는 주제에 도달했다: **같은 문자 `&`를 세 가지 다른 방식으로 인코딩하는 세계.**
...
- **이스케이프가 필요한 문자**: `"`, `\`, 제어 문자(U+0000~U+001F)
...
**올바른 레이어의 파서를 거쳐야 한다.** 레이어를 건너뛰면 인코딩이 해석되지 않고 깨진다.
...
1. **인코딩 레이어를 건너뛰지 마라.** JSON 응답에서 URL을 꺼낼 때는 JSON parser를 거쳐야 한다.
...
인코딩 문제를 볼 때는 raw bytes를 따로 확인해야 한다. httpie의 예쁜 출력은 이미 JSON parser를 지난 결과라서, `\u0026`처럼 원본에 있던 escape를 숨길 수 있다.
