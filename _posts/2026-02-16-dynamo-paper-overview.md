...
ACID를 보장하는 저장소는 가용성이 떨어지는 경향이 있다. Dynamo는 높은 가용성을 얻기 위해 약한 일관성(ACID에서 "C")을 허용한다. 여기서 일관성이란 모든 노드가 동일한 데이터를 보도록 보장하는 성질이다. Dynamo는 **최종 일관성(Eventual Consistency)** — 지금은 노드마다 다를 수 있지만, 시간이 지나면 결국 같아지는 모델 — 을 채택했다. 고립성(Isolation)을 보장하지 않으며, 단일 키 업데이트만 허용한다.

고립성을 보장하지 않지만 단일 키 업데이트만 허용하므로, 여러 키에 걸친 트랜잭션 자체가 없어서 문제가 단순해진다.

**효율**

99.9분위에서 측정하는 엄격한 레이턴시 SLA를 따른다. 평균이 아니라 꼬리 지연(tail latency)을 기준으로 삼는다는 점이 중요하다.
