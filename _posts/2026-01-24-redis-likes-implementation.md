...
뉴스 댓글에 좋아요 기능을 추가한다고 가정해봅시다. 실시간 트래픽이 많은 사이트라면 1초에 수만 개의 좋아요가 발생할 수 있습니다. RDB만으로 충분할까요?

## 먼저 생각해볼 것

> - 초당 좋아요 요청이 얼마나 될 것으로 예상하는가?
> - 같은 사용자가 여러 번 좋아요를 누를 수 있는가? (중복 방지 필요?)
> - 좋아요 데이터가 유실되면 비즈니스에 치명적인가?
> - 실시간 정확성이 필요한가, 몇 초 지연은 허용되는가?
...
이 쿼리는 간단하지만, 트래픽이 많으면 심각한 **락 경합(Lock Contention)** 문제를 일으킵니다.

### InnoDB의 잠금 메커니즘

`UPDATE` 쿼리 실행 시 내부 동작:
...
모든 `UPDATE`가 순차 실행되어 처리량이 급격히 떨어집니다.

### 해결책 A: 카운터 샤딩 (Counter Sharding)

여러 개의 보조 카운터로 업데이트 요청을 분산시킵니다.
...
실시간 정확성 대신 **결과적 일관성(Eventual Consistency)**을 가집니다.
...
Redis는 항상 최신 실시간 카운트를 제공하고, RDB의 `like_count`는 결과적 일관성을 가집니다.

## 고려사항

### Redis Persistence

`RDB 스냅샷`이나 `AOF` 설정으로 데이터 유실을 방지할 수 있습니다.
...
> - Redis 업데이트 성공, RDB 실패 시 어떻게 처리하는가?
> - 좋아요 수의 정합성을 검증하는 배치 작업이 있는가?
> - Redis 장애 시에도 서비스가 동작하도록 폴백이 있는가?
