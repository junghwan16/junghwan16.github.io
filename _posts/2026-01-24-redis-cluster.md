...
단일 Redis로 충분한가? 메모리가 부족하거나 처리량이 한계에 달했다면 클러스터를 고려할 때다. 핵심은 **해시 슬롯 16384개**로 데이터를 분산하고, **MOVED/ASK 리다이렉트**를 통해 클라이언트가 올바른 노드로 재시도한다는 점이다.
...
단일 Redis 인스턴스가 한계에 다다르면 두 가지 선택지가 있다.
...
키를 CRC16으로 해시한 후 16384로 나눈 나머지로 슬롯이 결정된다.
...
클러스터에서는 **다중 키 커맨드** (예: `MGET`)를 기본적으로 사용할 수 없다.
...
`{}` 안의 값만으로 해시되어 같은 슬롯에 저장된다.

> 주의: 너무 많은 키가 같은 해시태그를 가지면 특정 슬롯에 데이터가 몰릴 수 있다.

## MOVED vs ASK

클라이언트가 잘못된 노드에 요청하면 리다이렉트 응답을 받는다.
...
클러스터를 도입하기 전에 다음 제약사항을 반드시 파악해야 한다.
...
모든 노드가 **풀 메쉬(Full Mesh)** 구조로 연결된다.
...
**가십 프로토콜**을 사용해 노드가 증가해도 메시지가 기하급수적으로 늘어나지 않는다.

## 자동 재구성

### 자동 페일오버

마스터 장애 시 복제본을 마스터로 승격하는 과정은 다음과 같다:
...
운영 환경에서는 보통 **3 마스터 + 3 복제본 = 6노드**로 구성한다.
...
슬롯 이동 중 해당 슬롯 접근 시 지연이 발생할 수 있다.
...
- [Redis Cluster - redis-py docs](https://redis-py.readthedocs.io/en/stable/clustering.html)
